...
export default class InternalDateMediator {
  ...
  public validateString = (value: string): boolean => InternalDateTransformer.parseValueToDate(value) !== null;
  ...
  public getSelected = (selected: InternalDateComponentType | null): string => {
    if (selected === InternalDateComponentType.All) {
      return this.iDate.toString({ withPad: true });
    }
    const value = this.iDate.get(selected);
